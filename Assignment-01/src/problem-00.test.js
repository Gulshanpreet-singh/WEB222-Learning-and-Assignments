 ...
 * has already been written for you, you don't need to change anything
 * in these files, just read what's here.  They include tests for the assignment's
 ...
 * have to write the tests, only an implementation of a function in order
 * to have them pass.
 *
 ...
 * us to group tests for a particular feature together and give them a name.
 * Doing so makes it a bit easier to see what's going on when we run our tests.
 * We also define a test() {...} function to contain our tests.  When the problem-00
...
describe('Problem 0 - greeting() function', function () {
   ...
   * small, and only test one thing at a time.  This makes it easier to implement
   ...
   * a name (i.e., a string), that it returns a new string formatted in the
   * expected way.
   */
