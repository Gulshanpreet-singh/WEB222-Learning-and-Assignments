...
function observationSummary2(data) {
  data.results.forEach((elem) =>
    console.log(`#${elem.id} - ${elem.species_guess} [${elem.observed_on_details.date}]`)
  );
}
...
function observationsByPrivacy(data, privacy) {
  if (
    !(privacy === null || privacy.toLowerCase() === 'open' || privacy.toLowerCase() === 'obscured')
  ) {
    throw 'ERROR';
  } else {
    let arr = [];
    ...
    for (const element of data.results) {
      if (privacy === null) {
        ...
        if (element.privacy === privacy.toLowerCase()) {
          arr.push(element);
        }
      }
    }
...
function transformObservations2(data) {
  let transformedArray = data.results.map((element) => transformObservation(element));
...
function getObservationsById(data, ...ids) {
  if (ids.length === 1) {
    let found = data.results.find((element) => element.id === ids[0]);
  ...
  ids.forEach(function (id) {
    let found = data.results.find((element) => element.id === id);
    if (found) foundArr.push(found);
  });
  return foundArr;
}
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  ...
  if (options.gte && options.lte && options.eq) {
    observations = data.results.filter((element) => element.positional_accuracy === options.eq);
  } else if (options.gte && options.lte) {
    observations = data.results.filter(
      (element) =>
        element.positional_accuracy >= options.gte && element.positional_accuracy <= options.lte
    );
  } else if (options.gte) {
    observations = data.results.filter((element) => element.positional_accuracy >= options.gte);
  } else if (options.lte) {
    observations = data.results.filter((element) => element.positional_accuracy <= options.lte);
  } else if (options.eq) {
    observations = data.results.filter((element) => element.positional_accuracy === options.eq);
  } else {
...
function getTaxonPhotos(data) {
  let returnArr = [];
  for (let i = 0; i < data.results.length; i++) {
    if (!data.results[i].taxon) {
      return [];
    }
    if (data.results[i].taxon.default_photo !== null) {
      let url = 'https://static.inaturalist.org/photos/575727/';

      let photoObject = {
        original: url + 'original.jpg?' + data.results[i].id,
        square: url + 'square.jpg?' + data.results[i].id,
        small: url + 'small.jpg?' + data.results[i].id,
        medium: url + 'medium.jpg?' + data.results[i].id,
        large: url + 'large.jpg?' + data.results[i].id
      };

      returnArr.push(photoObject);
    }
  }
...
function getUserStats(data) {
  let user_count = data.results.length,
    observations_count = 0,
    journals_count = 0,
    species_count = 0;

  for (let i = 0; i < data.results.length; i++) {
    observations_count += data.results[i].user.observations_count;
    journals_count += data.results[i].user.journal_posts_count;
    species_count += data.results[i].user.species_count;
  }
  ...
  let returnObj = {
    ...
    averages: {
      observations: observations_count / user_count,
      journals: journals_count / user_count,
      species: species_count / user_count
    }
  };
